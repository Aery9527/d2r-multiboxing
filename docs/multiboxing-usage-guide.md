...
| `GraphicsProfile` | 可先留空 | 這個帳號要套用的具名畫質設定檔；建議用主選單 `g` 產生與指派。若留空，啟動時工具不會碰 `Settings.json`。之後啟動指定帳號或 `a` 批次啟動時，進入畫質選單直接按 Enter 就會使用這裡的設定；若本次目標裡有人尚未設定有效的預設畫質設定檔，工具會擋下並列出帳號。若你想這次完全不動 `Settings.json`，可在畫質選單手動選 `0` |
...
- 若本次目標裡有任何帳號尚未設定有效的 `GraphicsProfile`，工具會直接擋下並列出那些帳號，讓你改成手動選 `0` / 其他畫質設定檔，或先回主選單補設定
