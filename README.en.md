...
- If you have assigned `GraphicsProfile` values to your accounts, pressing Enter in the launch graphics menu uses those saved defaults for single-account and batch launch. If any targeted account is still missing a valid default graphics profile, the launcher blocks Enter, lists the affected accounts, and lets you either go back to `g` to configure them or choose `0` / another graphics-profile number manually.
