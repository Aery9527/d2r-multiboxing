...
d2r-hyper-launcher — 在同一台 Windows 電腦上同時執行多個 Diablo II: Resurrected (D2R) 實例的輔助工具。
...
- [cmd/d2r-hyper-launcher/main.go](../../cmd/d2r-hyper-launcher/main.go) - CLI 互動主迴圈
- [internal/config/](../../internal/config/) - 設定檔管理（`~/.d2r-hyper-launcher/config.json` 讀寫）
...
- 導航選項值在整個專案內統一使用常數 `menuBack = "b"`（回上一層）、`menuHome = "h"`（回主選單）、`menuQuit = "q"`（離開程式），定義於 [cmd/d2r-hyper-launcher/main.go](../../cmd/d2r-hyper-launcher/main.go)
