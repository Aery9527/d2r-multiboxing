...
- `<數字>`：先進入 region 選單；若該帳號已設定 `DefaultRegion`，可直接按 Enter 使用預設值，也可手動選 `1` / `2` / `3` 覆蓋這次啟動。接著再進入 mod 選單；若該帳號已設定 `DefaultMod`，也可直接按 Enter 使用預設值，或手動選 `0` / 其他編號覆蓋這次啟動。最後才會啟動指定帳號；若該帳號已在執行中會直接阻止重複啟動
- `a`：工具會先預掃描目前已開啟帳號；如果還有待啟動帳號，才會再讓你選區域與 mod。區域選單可直接按 Enter 讓每個待啟動帳號吃自己的 `DefaultRegion`，mod 選單也可直接按 Enter 讓每個待啟動帳號吃自己的 `DefaultMod`；你也可以手動選 `1` / `2` / `3` 或 `0` / 其他 mod 編號，替本次批次全部帳號做單次覆蓋
